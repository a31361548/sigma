...
**本專案備註**：
* 理專/客戶/帳號 icon 路徑：`/image/gangs.png`、`/image/person.png`、`/image/accounts.png`
* 初始載入可能先顯示純色圓點，圖示載入完成後會切換為 icon（正常行為）
...
1) 安裝：`npm i @sigma/edge-curve`

2) 註冊 edge program（sigma settings）：
...
* edge attrs 設 `type: "curved"`
* edge attrs 加上 `curvature: number`（不同邊用不同曲率，才不會重疊）

補充：如果你是用 `@react-sigma/core`，要允許同一組 `A->B` 多條邊，`SigmaContainer` 內部的 graph 也必須是 multi（否則 `graph.import(...)` 會丟 `edge already exists`）。做法是傳入自訂 graph constructor，例如：
...
<SigmaContainer graph={MultiDirectedGraph} settings={{ edgeProgramClasses: { curved: EdgeCurveProgram } }} />
```
...
**本專案備註**：
* 下載：`downloadAsPNG(renderer, { fileName: "graph", backgroundColor: "#ffffff" })`
* 以目前畫布尺寸輸出（未額外指定 width/height）
