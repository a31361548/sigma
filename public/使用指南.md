...
* [15. 多中心群（理專 → 客戶 → 帳戶）：ELK 排版策略](#15-多中心群理專--客戶--帳戶elk-排版策略純策略---可直接落地)
  ...
  * [15.5 初始載入：自動選中心群](#155-初始載入自動選中心群的指標由我用指標選穩定可解釋)
  * [15.6 TypeScript 介面](#156-可直接落地的-typescript-介面不綁-react不用-any)
  * [15.7 Sigma 端怎麼用](#157-sigma-端怎麼用概念)
...
> **⚠️ 重要注意：標籤渲染器的優先權陷阱**
>
> 在某些版本或設定下，Sigma 可能會優先讀取 `defaultDrawNodeLabel` 與 `defaultDrawNodeHover` 作為預設繪圖函式。
> 如果你發現設定了 `labelRenderer` 卻無效（例如文字位置沒變），請嘗試**同時強制覆寫**以下所有屬性指向你的自訂函式：
> ```ts
> {
>   labelRenderer: myDrawLabel,
>   hoverRenderer: myDrawLabel,
>   defaultDrawNodeLabel: myDrawLabel,
>   defaultDrawNodeHover: myDrawLabel
> }
> ```
...
## 補充：@react-sigma / Settings 參數 / 事件 Payload / 動畫（依你補充的四點整理）
...
如果你希望我把 **「群內 ELK layout（layered）+ 群間 grid packing + Sigma camera 對準群 bbox」**寫成一個完整的可複製 util（含型別、純 TS、無任何框架依賴），我也可以直接把它補在這份 canvas 後面。
