...
### 7.8 空白處雙擊新增文字註記
- 事件：`doubleClickStage` → 開啟 textarea overlay：`src/components/Graph/SigmaCanvas.tsx`
- 確認後新增節點（不連邊）：
  - `isFreeText: true`、`labelColor: "#dc2626"`、`labelMaxWidth`（依字數與 size 推估）、`forceLabel: true`：`src/components/Graph/SigmaCanvas.tsx`
- label 繪製支援單一節點字色與換行寬度：`src/utils/sigma/drawLabel.ts:1`
