...
### 1.2.4 空白處雙擊文字註記（Free Text）

位置：`src/components/Graph/SigmaCanvas.tsx`

- 觸發：`doubleClickStage` 事件開啟 textarea overlay（含確認/取消）
- 節點屬性：
  - `isFreeText: true`
  - `labelColor: "#dc2626"`（紅色字）
  - `labelMaxWidth`（依字數與 size 推估，用於自動換行）
  - `forceLabel: true`（強制顯示 label，不受 labelDensity 抽樣影響）
- 節點樣式：節點本體顏色透明（`rgba(0, 0, 0, 0)`），僅顯示文字
...
**新增資料要能匯出**
- 若是「節點/邊屬性」：直接存到 graph 的 node/edge attributes，`exportGraph` 會包含。
- 若是「UI 狀態」：新增欄位到 `uiState`，並在匯出/匯入時同步解析。

**匯入解析清單**
- `markedNodeIds` → `Set<string>`
- `notesByNodeId` → `Map<string, string>`
- `notePositions` → `Map<string, { x: number; y: number }>`
- `expandMode` / `layoutMode` / `camera` → 直接還原
